...
namespace FootlooseFS.DataPersistence.Doc
{
    ...
    {
        public PersonDocumentRepository(IMongoDatabase database, string tableName) : base(database, tableName) { }

        protected override IOrderedQueryable<PersonDocument> ApplyFilter(PersonDocument searchCriteria, string sort, FootlooseFS.Models.SortDirection sortDirection)
        {
            var queryable = GetQueryable();

            if (searchCriteria.Id > 0)
                queryable = queryable.Where(p => p.Id == searchCriteria.Id);
            ...
            if (sortDirection == FootlooseFS.Models.SortDirection.Ascending)
            {
                if (sort == "firstname")
                    ...
                    orderedQueryable = queryable.OrderBy(p => p.Id);
            }                
            else
            {
                if (sort == "firstname")
